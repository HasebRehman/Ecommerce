...
const STATUS_CONFIG = {
  ...
  approved: {
    ...
    label: 'Approved! 🎉',
    desc:  'Congratulations! You are now a Retailer. Login again to access your dashboard.',
  },
  rejected: {
    icon:  XCircle,
    color: 'text-red-400',
...
export default function RequestSellerPage() {
  ...
  const loadRequest = async () => {
    try {
      const response = await api.get(API.STORE.ROLE_REQUEST)
  ...
  useEffect(() => {
    ...
    // Listen for role_upgrade_requests changes
    ...
    // Listen for user_roles changes
    const roleSub = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (newRole === 'business_owner') {
            toast.success('Role updated! Please login again to access your dashboard.')
          }
        }
      )
      .subscribe()
  ...
  const handleSubmitRequest = async () => {
    ...
    setSubmitting(true)
    try {
      const response = await api.post(API.STORE.ROLE_REQUEST)
      setRequest(response.data.request)
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <h1 className="text-2xl font-bold text-white">
          You're already a Retailer!
        </h1>
        <p className="text-slate-400 mt-2 mb-6">
          Access your dashboard to manage your shops and products
        </p>
        <button
    ...
    <div className="max-w-xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-slate-400 mt-2">
          Start selling your products on VendoSphere
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 mb-8">
        {[
          { icon: '🏪', title: 'Create Multiple Shops',   desc: 'Set up shops for different product categories' },
          { icon: '📦', title: 'Manage Inventory',        desc: 'Add and manage your products easily' },
          { icon: '📊', title: 'Track Orders',            desc: 'Monitor your sales and orders in real-time' },
          { icon: '🚀', title: 'Go Live Anytime',         desc: 'Control when your shop is visible to customers' },
        ].map(benefit => (
          <div
            key={benefit.title}
            className="flex items-start gap-3 p-4 bg-slate-900 border border-slate-800 rounded-xl"
          >
      ...
      {/* Request status or submit button */}
      {!isAuthenticated ? (
        <div className="text-center space-y-3">
          <p className="text-slate-400 text-sm">
            You need to be logged in to request seller access
          </p>
          <button
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-3">
            {(() => {
              ...
              return <Icon className={cn('w-6 h-6', statusConfig.color)} />
            })()}
            <div>
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-slate-700">
            ...
            <span className="text-slate-400 text-xs">
              Live updates enabled — no need to refresh
            </span>
          </div>

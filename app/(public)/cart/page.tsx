...
export default function CartPage() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-white text-xl font-bold">Login to view your cart</p>
        <p className="text-slate-400 mt-2 mb-6">You need to be logged in to view cart items</p>
        <Link href="/login">
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-slate-400 mt-2 mb-6">Start adding products to your cart</p>
        <Link href="/">
          <button className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2.5 rounded-xl font-medium transition-colors">
            Continue Shopping
          </button>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-white mb-6">My Cart ({count} items)</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {items.map(item => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-4 p-4 bg-slate-900 border border-slate-800 rounded-xl">
                <div className="w-20 h-20 rounded-lg overflow-hidden bg-slate-800 shrink-0">
                  {product.images?.[0]
                    ? <img src={product.images[0]} alt={product.name} className="w-full h-full object-cover" />
                    : <div className="w-full h-full flex items-center justify-center"><ShoppingBag className="w-8 h-8 text-slate-600" /></div>
                  }
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-white font-medium text-sm line-clamp-2">{product.name}</p>
                  <p className="text-white font-bold mt-1">Rs. {displayPrice.toLocaleString()}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="w-7 h-7 rounded-lg bg-slate-800 hover:bg-slate-700 text-white flex items-center justify-center disabled:opacity-40"
                  >
                    <Plus className="w-3 h-3" />
                  </button>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-white font-bold text-sm">Rs. {(displayPrice * item.quantity).toLocaleString()}</p>
